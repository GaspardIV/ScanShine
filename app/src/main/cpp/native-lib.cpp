...
bool cornersAreNotCriticalCorners(std::vector<Point> &points, const Mat &mat) {
    for (Point &p : points) {
        if (isNear(p, Point{0, 0}, NEAR_DIST)   /*(p.x == 0 && p.y == 0)*/
            ...
            || isNear(p, Point{mat.cols - 1, mat.rows - 1}, NEAR_DIST) /*(p.x == mat.cols - 1 && p.y == mat.rows -1))*/) {
            return false;
        }
    }
    return true;
    ...
    for (int c = 0; c < 3; c++) {
        ...
        for (int l = 0; l < N_SHIFTS; l++) {
            ...
            for (const auto &contour : contours) {
                approxPolyDP(Mat(contour), approx, arcLength(Mat(contour), true) * APPROX_POLY_EPSILON, true);
                ...
                if (approx.size() == 4 &&
                    ...
                    for (int j = 2; j < 5; j++) {
                        maxCosine = MAX(maxCosine, fabs(cosine(approx[j % 4], approx[j - 2], approx[j - 1])));
                    }
            ...
            for (int j = 0; j < squares.size(); ++j) {
                if (i != j && weight[j] > 0) {
                    if (isNear(squares[i][0], squares[j][0], NEAR_QUAD) &&
                        ...
                        squares[i][0].x = static_cast<int>((weight[i] * squares[i][0].x + weight[j] * squares[j][0].x) /
                                                           (double) (weight[i] + weight[j]));
                        squares[i][0].y = static_cast<int>((weight[i] * squares[i][0].y + weight[j] * squares[j][0].y) /
                                                           (double) (weight[i] + weight[j]));

                        squares[i][1].x = static_cast<int>((weight[i] * squares[i][1].x + weight[j] * squares[j][1].x) /
                                                           (double) (weight[i] + weight[j]));
                        squares[i][1].y = static_cast<int>((weight[i] * squares[i][1].y + weight[j] * squares[j][1].y) /
                                                           (double) (weight[i] + weight[j]));

                        squares[i][2].x = static_cast<int>((weight[i] * squares[i][2].x + weight[j] * squares[j][2].x) /
                                                           (double) (weight[i] + weight[j]));
                        squares[i][2].y = static_cast<int>((weight[i] * squares[i][2].y + weight[j] * squares[j][2].y) /
                                                           (double) (weight[i] + weight[j]));

                        squares[i][3].x = static_cast<int>((weight[i] * squares[i][3].x + weight[j] * squares[j][3].x) /
                                                           (double) (weight[i] + weight[j]));
                        squares[i][3].y = static_cast<int>((weight[i] * squares[i][3].y + weight[j] * squares[j][3].y) /
                                                           (double) (weight[i] + weight[j]));
...
bool findBiggestSquere(std::vector<std::vector<Point>> &squares, const int weight[], std::vector <Point>& biggest) {
    bool found = false;
...
Java_com_gaspard_scanshine_views_CameraView_decode(JNIEnv *env, jclass type,
    ...
    Mat mImage(height + height /2, width, CV_8UC1, (unsigned char *) yuv420sp);
    cvtColor( mImage, mImage, COLOR_YUV420sp2RGBA);
    ...
    if (findBiggestSquere(squares, weight, biggest)) {
        arr[0] = static_cast<jint>(biggest[0].x * ratio);       arr[1] = static_cast<jint>(biggest[0].y * ratio);
        arr[2] = static_cast<jint>(biggest[1].x * ratio);       arr[3] = static_cast<jint>(biggest[1].y * ratio);
        arr[4] = static_cast<jint>(biggest[2].x * ratio);       arr[5] = static_cast<jint>(biggest[2].y * ratio);
        arr[6] = static_cast<jint>(biggest[3].x * ratio);       arr[7] = static_cast<jint>(biggest[3].y * ratio);
    } else {
        arr[0] = static_cast<jint>(0);                          arr[1] = static_cast<jint>(0);
        arr[2] = static_cast<jint>(0);                          arr[3] = static_cast<jint>(rows - 1);
        arr[4] = static_cast<jint>(cols - 1);                   arr[5] = static_cast<jint>(rows - 1);
        arr[6] = static_cast<jint>(cols - 1);                   arr[7] = static_cast<jint>(0);
    }
